...
class Shooters {
  ...
  //rotation speeds
  FloatList rotations = new FloatList(.005, -.005, .01, .0090908, -.004, .00523, .015123, -.026, -.007, .009, -0.01, 0.05, 0.009, -0.009);
  ...
  void update() {
    ...
    for (int i = 20; i<200; i+=20) {
      ...
      if (temp == i) {
        ...
        enemies++;
        //shuffle rotation speeds for the shooters
        rotations.shuffle();
      }
    }
    //spawning bullets
    ...
    for (int i = 0; i<bullet.length; i++) {
      ...
      //turn down bgm play out of bounds music make all spawned bullets go directly towards you
      if (out) {
        vol = 0;
        if (!bound.isPlaying()) {
          bound.play(random(0.03, 0.5));
        }       
        ball.stop();
